...
| http://marketto.ru/page/tinymce-images-plugin
| 
...
/*-------------------------------------------------------------------
|
| Язык интерфейса скрипта загрузки изображений.
| Рекомендуем выбрать тот, что используется в tinyMCE. Доступны варианты:
...
| Пример: если изображение доступно по адресу http://www.site.com/images/somefolder/image.jpg, то укажите здесь:
| 
| $config['img_path'] = '/images/somefolder';
| 
-------------------------------------------------------------------*/

	/*if(eregi('/news/',$PHP_SELF) || eregi('/pressabout/',$PHP_SELF)) $config['img_path'] = '/images/db/news';
	else*/ $config['img_path'] = '/images/db/news';
...
/*-------------------------------------------------------------------
| 
| Преобразовывать русские символы в именах загружаемых изображений в латинские
| `TRUE` - преобразовывать
| `FALSE` - нет (совместимость с русскими именами не гарантируется)
| 
-------------------------------------------------------------------*/


	$config['latinize_name'] = TRUE;
